...
runner.bench_time_func("delete", benchmark_delete, cachebox.LRUCache(0))
...
runner.bench_time_func("clear", benchmark_clear, cachebox.LRUCache(0))

# Get
runner.timeit(
    "get 100 items",
    "for i in range(100): cache.get(i)",
    "cache = cachebox.LRUCache(100); cache.update({i:i for i in range(100)})",
    globals=globals()
)
